...
// Data storage
const _majorLookup = {};
const _completeMajorMap = [];
const _currentCampus = "Seattle";
let _timer;// Timer to hide search suggestions after period of inactivity - used in major.js

const _campusNameLookup = {};
const _majorNameLookup = {};
const _courseNameLookup = {};
const _statusLookup = {};
const update_results_on_load = false;
let all_data_loaded = false;
let _searchResultsChecked = false;

// a list of listeners waiting for the response
const statusLookupListener = [];
...
function initOnboardingDialog() {
    ...
    const isTempForgotten = sessionStorage.getItem("isTempForgotten");
    ...
    if (isTempForgotten == null) {
        ...
        let isPermForgotten = localStorage.getItem("isPermForgotten");
        ...
        if (isPermForgotten == false || isPermForgotten == "false") {
            ...
            $("#onboard-modal").on("shown.bs.modal", () => {
    ...
    $("#perm-forget-modal").on('keydown', (e) => {
    ...
    $("#onboard-modal").on("hidden.bs.modal", () => {
        sessionStorage.setItem("isTempForgotten", true);
        $("#shortcut").focus();
    });

    $("#perm-forget-modal").on("click", () => {
...
function getDataNameMap(queryStr) {
    ...
    queryStr = queryStr || "";
    d3.csv(`/api/v1/data_map/${queryStr}`, (d) => {
        return {
...
function getCompleteMajorMap(queryStr) {
    queryStr = queryStr || "";
    d3.csv(`/api/v1/major_course/${queryStr}`, (d) => {
        return {
        ...
        for (const index in data) {
            const major = data[index].major_abbr.replace(/_/g, "-");

            // Format of a course_number is MATH_126, splitting it into MATH and 126
            const splitIndex = data[index].course_number.lastIndexOf("_");
            const dept_abbrev = data[index].course_number.substring(0, splitIndex);
            const course_number = data[index].course_number.substring(splitIndex + 1);
            const cID = dept_abbrev + course_number;
...
function getMajorStatus(queryStr) {
    queryStr = queryStr || "";
    d3.csv(`/api/v1/status_lookup/${queryStr}`, (d) => {
        return {
            code: d.code.trim(),
        ...
        for (const index in data) {
            const code = data[index].code.replace(/_/g, "-");
        ...
        if (statusLookupListener.length > 0) {
            statusLookupListener.map((listener) => {
                listener();
            });
        }
...
function displayMajorStatusURL(code) {
    ...
    let major_abbr = parts[0].replace('-', ' ');

    if (myplan_alias[major_abbr]) {
	   major_abbr = myplan_alias[major_abbr][0];
    }

    const url = `https://myplan.uw.edu/program/#/programs/UG-${  major_abbr  }-MAJOR`;
    let msg = _completeMajorMap[code].major_full_nm;
    const source = $("#display-major-status-url").html();
...
function displayMajorStatusIcon(code) {
    ...
    if (_statusLookup.hasOwnProperty(code)) {
        const title = _statusLookup[code].status;
        const url = images_paths[title];
        // Compile the Handlebar template
        const source = $("#display-major-status-icon").html();
        const template = Handlebars.compile(source);
...
function addStudents(queryStr) {
    queryStr = queryStr || "";
    d3.csv(`/api/v1/student_data/${queryStr}`, (d) => {
        return {
            major_abbr: d.major_path.trim(),
        ...
        if (window.location.pathname == '/major-gpa/' && getParameterByName("code") != null) {
            const majorParam = getParameterByName("code").split(",");
            const validCodes = [];
            const invalidCodes = [];

            for (const i in majorParam) {
...
function init_search_events() {
    ...
    $("#search").on("focus", () => {
        ...
        const placeholder = /[0-9]+\smajor[s]*\sselected/.test(search_val);
    ...
    $(".dropdown-menu").keydown((e) => {
        if (e.which == 40 || e.which == 39) { // down or right arrow key {
            ...
            const firstChild = curFocused.find(".college-list").first()[0];
            // next college
            const nextCollege = curFocused.next(".college-list")[0];
            // next campus
            const nextCampus = 
            ...
            const lastChild = curFocused.find(".college-list").last()[0];
            // prev college
            const prevCollege = curFocused.prev(".college-list")[0];
            // prev campus
            const prevCampus = 
                curFocused.parents(".dropdown-header").prev(".divider").prev(".dropdown-header")[0] ||
                curFocused.prev(".divider").prev(".dropdown-header")[0];
            // all colleges option
            const allColleges = $("#college-opt1");
    ...
    $("#search").keydown((e) => {
        ...
        const inputs = $("#suggestions .suggested_major input");
        // if the input exists, use it, otherwise use the li
        const suggestedMajor = inputs.length ? inputs : $("#suggestions .suggested_major");
...
function prepareResults(e) {
    ...
    const raw_search = $("#search").val().replace('(','').replace(')','');

    if (raw_search == "") {
        const results = $('.suggested_major').length;
        let college_suggestions;
...
function finishResults() {
    ...
    const source = $("#finish-results").html();
    const template = Handlebars.compile(source);
    const source_divider = $("#finish-results-divider").html();
    const template_divider = Handlebars.compile(source_divider);
...
function populateCollegeDropdown() {
    ...
    $("#college-dropdown .dropdown-menu li.college-list").click(function(){
        ...
        const prev_selected = $("#college-dropdown .dropdown-menu .active");
        ...
        const selection_source = $("#populate-college-dropdown-college-selection").html();
        const selection_template = Handlebars.compile(selection_source);
        ...
        const campus = $(this.classList).filter(["Seattle", "Bothell", "Tacoma"])[0];
    ...
    if (window.location.pathname == '/course-gpa/' && getParameterByName("code") != null) {
        const major = getParameterByName("code").replace("_", " ").trim();
...
function clearCollegeSelection() {
    const source = $("#populate-college-dropdown-college-selection").html();
    const template = Handlebars.compile(source);
...
function addCapacityDescription(id, location) {
    const source = $("#admission-type-help-popover").html();
    const template = Handlebars.compile(source);
    const clear_id = id.replace("_", " ");
...
$('#search').on('keyup.num focus.num', (e) => {
