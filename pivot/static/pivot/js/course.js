...
function checkStoredData() {
    if (sessionStorage.length > 0 && sessionStorage.getItem("courses") != null && sessionStorage.getItem("courses") != "null") {
        const majorCode = sessionStorage.getItem("courses");
...
function displayResults() {
    ...
    const source = $("#display-results").html();
    const template = Handlebars.compile(source);

    let count = 0;
    const search_val = $("#search").val().toLowerCase().replace('(','').replace(')','').replace(/\s+/g,' ').trim();

    for(const maj in _completeMajorMap) {
        // If the search term matches the full name of the major
        const index = _completeMajorMap[maj].major_full_nm.toLowerCase().indexOf(search_val);
        // If the search term matches the major abbreviation
        const abbr_index = maj.split('-')[0].toLowerCase().indexOf(search_val);
        // If the search term matches an alias (listed in alias.js)
        let alias_index = false;
        ...
        if (search_val.length > 0 && (alias_index || abbr_index == 0 || (index > -1 && (index == 0 || _completeMajorMap[maj].major_full_nm.toLowerCase().charAt(index - 1) == " " || _completeMajorMap[maj].major_full_nm.toLowerCase().charAt(index - 1) == "(")))) {
            ...
            const substring = _completeMajorMap[maj].major_full_nm.substr(index, search_val.length);
            var appendTo = "";
...
function showCurrentSelections() {
    ...
    $(".chosen_major").each(function() {
        ...
        const majText = _completeMajorMap[$(this).text()].major_full_nm;
...
function noResults() {
    ...
    const template = Handlebars.compile(source);
    const search_key = $("input#search").val();
...
function multipleResults() {
    ...
    $(".protected-result-warning").css("display","none");
    $(".no-results-warning").css("display","inline");
    const source = $("#multiple-results").html();
    const template = Handlebars.compile(source);
    const search_key = $("input#search").val();
...
function listCoursesForMajor(maj) {
    ...
    const studentCounts = Object.values(courses).map((value) => { 
        return value.student_count;
    });

    if(studentCounts.some((k) => { return ~k.indexOf("-1") })){
       protectedResult(maj);
    ...
    const inner_table_data = [];

    for (const c in courses) {
        // Show only upto 10 results
        if (count < 10) {
            const popularity = `${Math.round((courses[c].student_count / _completeMajorMap[maj].students_in_major) * 100)  }%`;
            const col = `q${  colorBucket(courses[c].percentiles[5]) }-9`;
            const percentile = round(parseFloat(courses[c].percentiles[5]), 2);
            // replace all spaces not before a number with a dash
            // add a space before the number
            const name_ref = c.replace(/\s(?!(\d))/, "-").replace(/(\d+)/, '_$1');
    ...
    const source = $("#list-courses-for-major-outer").html();
    const template = Handlebars.compile(source);
